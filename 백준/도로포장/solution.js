...
for(let i = 1; i <= n; i++) distance[i] = new Array(k + 1).fill(Infinity);
...
function dijkstra(start) {
    ...
    while(queue.size() !== 0) {
        const [current, dist, paved] = queue.deq();
        if(distance[current][paved] < dist) continue;
